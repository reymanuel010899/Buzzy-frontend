...
public class VideoPagerAdapter extends RecyclerView.Adapter<VideoPagerAdapter.PageHolder> {
    ...
    // El player ACTIVO ya pintó el PRIMER frame del video cargado por playUrl. El
    // carrusel horizontal lo usa para revelar el ExoPlayer (desvanecer el thumbnail)
    // SOLO cuando el frame del slide correcto ya está visible → cero "pantallazo" del
    // frame del slide vecino mientras prepare() aún bufferea el nuevo video.
    interface FrameCallback { void onFirstFrame(int position); }
    private FrameCallback frameCallback;
    void setFrameCallback(FrameCallback cb) { this.frameCallback = cb; }
    ...
    void replaceUrls(ViewPager2 pager, List<String> newUrls) {
        ...
        // Volver a la página 0 y activarla cuando el RecyclerView la tenga lista.
        if (pager != null) {
    ...
    void setScrollPausing(boolean pausing) {
        ...
        if (pausing) {
            ...
            if (activePos == scrollStartPos && musicPlayer != null && currentMusicUrl != null) {
    ...
    void setMusic(String url, float volMusic, float volOriginal, long trimStartMs, long trimEndMs) {
        ...
        if (url == null || url.isEmpty()) {
            // Este video no tiene música: parar la del anterior.
            if (musicPlayer != null) { musicPlayer.stop(); musicPlayer.clearMediaItems(); }
            currentMusicUrl = null;

...
const HeaderStories: React.FC<HeaderStoriesProps> = ({ inline = false, showLabels = true }) => {
  ...
  return (
    <div className={inline ? "flex flex-1 items-center overflow-hidden" : "w-full border-t border-white/5 bg-black/90 backdrop-blur-xl"}>
      <div className={inline ? "flex flex-1 items-center gap-3 overflow-x-auto px-2 py-1 snap-x scrollbar-hide" : "flex gap-3 overflow-x-auto px-2 sm:px-6 py-2.5 snap-x scrollbar-hide"}>
        {groups.map((story, i) => (
          <motion.button
            key={story.id || i}
            type="button"
            ...
            className="relative flex min-w-[40px] flex-col items-center gap-1 snap-start group"
